 ...
 *
 *  Datei:          FtLibDemo1Dlg.cpp : Sample for FtLib.DLL - File with MS-VC++ Ver. 6.0
 *										
 *    
 *  Start:          30.04.2005
...
BOOL CFtLibDemo1Dlg::OnInitDialog()
{
    ...
    //	sIF.bUSBConnect = FALSE;			// 1=USB aktiv
        ...
        sIF.dwDistanzMode = IF_DS_INPUT_ULTRASONIC; // Ultrasonic Distance Sensor (08.2007: New)0;
    ...
    else
    {
        ...
        switch (dwStatus)
        {
            case ERROR_MOD_NOT_FOUND:
                sText.Format("FEHLER bei LoadFtLibDll(): 'FtLib.DLL' nicht gefunden! \nErrorCode: (#%d)", dwStatus);
                break;
            
            default:
                sText.Format("FEHLER bei LoadFtLibDll(); ErrorCode: (#%d)", dwStatus);
                break;
        }
...
void CFtLibDemo1Dlg::OnTimer(UINT nIDEvent) 
{
    ...
    //					        sIF.ucIRCodeL = sIF.pTransArea->IRKeys & 0x0F;					    // IR-Code "L" einblenden
		                ...
		                rdWert = sIF.uiVers * 0.01;		                                // in Volt umrechnen
		                text.Format("%2.2f Volt", rdWert);	    					    // und anzeigen ("Volt" angehängt)
